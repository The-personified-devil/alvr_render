...
extern "C" void rendr_take_vk(struct AlvrVkExport* exp) {
	*exp = expt;
}
    ...
    : Renderer(ctx.get_vk_instance(), ctx.get_vk_device(), ctx.get_vk_phys_device(), ctx.get_vk_queue_family_index(), ctx.get_vk_device_extensions())
{
    ...
    // TODO: Why is this how information is given to the renderer?

    m_quadShaderSize = len;
    m_quadShaderCode = reinterpret_cast<const uint32_t*>(shader);
    ...
    // TODO: Unfuck export to monado
    // i.e. no global shenaniganry :)
    ...
    // TODO: This should not be gotten that way
	...
	// TODO: Wait why is it used in two places?
		
        pipeline->SetShader((unsigned char*)shader, len);
...
{
    // TODO:
    std::cout << "we do not support color correction yet\n";
...
#undef ENTRY
    ...
    RenderPipeline *pipeline = new RenderPipeline(this);
    // TODO:
    std::cerr << "we do not support foveated encoding yet";
    ...
    m_pipelines.push_back(pipeline);
    // TODO: This is actually neat
    // But it should be named PipelinPart or something
    AddPipeline(pipeline);
}

// TODO: Why do we have multiple methods for shaders
