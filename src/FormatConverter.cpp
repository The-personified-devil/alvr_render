...
// TODO: Does this need to be a class like this?
FormatConverter::FormatConverter(Renderer *render) : r(render) {}
...
void FormatConverter::init(VkImage image, VkImageCreateInfo imageCreateInfo,
  ...
  VK_CHECK(
      vkAllocateCommandBuffers(r->m_dev, &commandBufferInfo, &m_commandBuffer));
  ...
  VK_CHECK(vkCreateDescriptorSetLayout(r->m_dev, &descriptorSetLayoutInfo,
  ...
  for (int i = 0; i < count; ++i) {
    ...
    memAllocInfo.memoryTypeIndex =
        r->memoryTypeIndex(memType, memReqs.memoryTypeBits);
    VK_CHECK(vkAllocateMemory(r->m_dev, &memAllocInfo, nullptr,
                              &m_images[i].memory));
    VK_CHECK(
        vkBindImageMemory(r->m_dev, m_images[i].image, m_images[i].memory, 0));
    ...
    viewInfo.components.a = VK_COMPONENT_SWIZZLE_IDENTITY;
    VK_CHECK(
        vkCreateImageView(r->m_dev, &viewInfo, nullptr, &m_images[i].view));
    ...
    vkCmdPipelineBarrier(r->m_commandBuffer,
    ...
    vkGetImageSubresourceLayout(r->m_dev, m_images[i].image, &subresource,
                                &layout);

    m_images[i].linesize = layout.rowPitch;
    VK_CHECK(vkMapMemory(r->m_dev, m_images[i].memory, 0, VK_WHOLE_SIZE, 0,
  ...
  VK_CHECK(vkCreatePipelineLayout(r->m_dev, &pipelineLayoutInfo, nullptr,
                                  &m_pipelineLayout));
  ...
  VK_CHECK(vkCreateComputePipelines(r->m_dev, nullptr, 1, &pipelineInfo,
                                    nullptr, &m_pipeline));
  ...
  r->d.vkCmdPushDescriptorSetKHR(
      m_commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, m_pipelineLayout, 0,
      descriptorWriteSets.size(), descriptorWriteSets.data());
...
uint64_t FormatConverter::GetTimestamp() {
  uint64_t query;
  VK_CHECK(vkGetQueryPoolResults(r->m_dev, m_queryPool, 0, 1, sizeof(uint64_t),
                                 &query, sizeof(uint64_t),
                                 VK_QUERY_RESULT_64_BIT));
  return query * r->m_timestampPeriod;
}

RgbToYuv420::RgbToYuv420(Renderer *render, VkImage image,
                         VkImageCreateInfo imageInfo, VkSemaphore semaphore)
    ...
    auto shader = new char[len];
    t.read(shader, len);

  init(image, imageInfo, semaphore, 3, (unsigned char*)shader,
       len);
}
